...
class MeetingMediaClipJobV1(Base):
    __tablename__ = "meeting_media_clip_jobs_v1"
    __table_args__ = (
        UniqueConstraint(
            "user_id",
            "client_clip_id",
            name="uq_media_clip_jobs_v1_user_client_clip",
        ),
        UniqueConstraint(
            "user_id",
            "idempotency_key",
            name="uq_media_clip_jobs_v1_user_idempotency",
        ),
        Index("idx_media_clip_jobs_v1_asset", "asset_id", "created_at"),
        Index("idx_media_clip_jobs_v1_meeting", "meeting_id", "created_at"),
    )
    ...
    user_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    data_epoch_id: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)
    client_clip_id: Mapped[str] = mapped_column(String(512), nullable=False)
    idempotency_key: Mapped[str] = mapped_column(String(512), nullable=False)
    request_hash: Mapped[str] = mapped_column(String(71), nullable=False)
    source_asset_revision: Mapped[int] = mapped_column(Integer, nullable=False)
    source_checksum_sha256: Mapped[str | None] = mapped_column(String(71), nullable=True)
