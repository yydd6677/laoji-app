...
const DOCS: Record<RootStackParamList['Legal']['kind'], {
  ...
  guide: {
    ...
    sections: [
      ...
      { heading: '会议资料', body: '登录账号使用按用户隔离的会议服务，并在本机缓存会议列表、转写和总结。访客会议主体保存在本机；使用语音转写或主动生成总结时，音频或转写会发送到对应服务，处理边界以隐私政策为准。服务不可用时，已缓存内容仍可查看，页面会提供明确错误和重试入口。' },
    ],
  },
  version: {
    title: '版本信息',
        ...
        {onlineLinks.length ? (
          <View style={s.onlineActions}>
            {onlineLinks.map(link => (
              ...
              >
                <Ionicons name={link.icon} size={16} color={link.icon === 'trash-outline' ? F.danger : F.primary} />
