...
export async function fetchMeetings(page = 1, size = 20): Promise<ApiMeeting[]> {
  const res = await fetch(meetingUrl(`/api/meetings?page=${page}&size=${size}`));
...
export async function updateMeeting(
  meetingId: string,
  changes: Partial<Pick<ApiMeeting, 'title' | 'description' | 'status' | 'participants'>>
): Promise<ApiMeeting> {
  const res = await fetch(meetingUrl(`/api/meetings/${meetingId}`), {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(changes),
  });
...
export async function fetchMeetingTranscript(meetingId: string): Promise<any[]> {
  const res = await fetch(meetingUrl(`/api/meetings/${meetingId}/transcripts?offset=0&limit=200`));
  if (!res.ok) return [];
  const data = await res.json();
  return data.items ?? [];
}
...
export async function fetchMeetingSummary(meetingId: string): Promise<string> {
  const res = await fetch(meetingUrl(`/api/meetings/${meetingId}/summaries/final`));
  if (!res.ok) return '';
  const data = await res.json();
  return (
    summaryValueToText(data.summary) ||
    summaryValueToText(data.content) ||
...
export async function fetchMeetingAudioInfo(meetingId: string): Promise<ApiMeetingAudioInfo | null> {
  const res = await fetch(meetingUrl(`/api/meetings/${meetingId}/audio-url`));
...
export async function deleteMeeting(meetingId: string): Promise<void> {
  const res = await fetch(meetingUrl(`/api/meetings/${meetingId}`), { method: 'DELETE' });
