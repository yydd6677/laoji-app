/**
 * Current Q2 single-reader execution boundary.
 *
 * It closes the snapshot/provider/grounding/publication boundary with an
 * injectable transport while keeping the local Q2 repository authoritative.
 */
...
export type Q2Source = Q2SnapshotSource & {
  text: string;
};

export interface Q2ProviderRequest {
  schemaVersion: 2;
  meetingId: string;
  snapshotId: string;
  ...
  question: string;
  operationId?: string;
  sources: readonly Q2Source[];
}

export interface Q2ProviderCitation {
  citationId: string;
...
export interface Q2ProviderClause {
  clauseId: string;
  answerStartUtf8: number;
  answerEndUtf8: number;
  citations: readonly Q2ProviderCitation[];
}

export interface Q2ProviderResponse {
  providerRequestId: string;
  providerRevision: string;
  answerKind: Q2AnswerKind;
  answer: string;
  clauses: readonly Q2ProviderClause[];
}

export interface Q2ExecutionResult {
  snapshotId: string;
  threadId: string;
  turnId: string;
  answerKind: Q2AnswerKind;
...
export interface Q2Provider {
  read(request: Q2ProviderRequest): Promise<Q2ProviderResponse>;
}
...
export async function buildQ2Sources(
  evidence: MeetingQuestionEvidence,
): Promise<Q2Source[]> {
  const sources = [
  ...
  if (sources.length < 1 || sources.length > MAX_Q2_SNAPSHOT_SOURCES) {
    throw new Error('Q2 来源数量超出当前上限');
  }
  return sources;
}

async function validateResponse(
  response: Q2ProviderResponse,
...
export async function executeQ2(input: {
  meetingId: string;
  evidence: MeetingQuestionEvidence;
  question: string;
  provider: Q2Provider;
  ...
  const request: Q2ProviderRequest = {
    schemaVersion: 2,
    meetingId,
    snapshotId: snapshot.snapshotId,
...
export { validateResponse as validateQ2Response };
