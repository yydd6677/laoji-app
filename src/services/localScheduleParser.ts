...
const CATEGORY_RULES: Array<[EventCategory, RegExp]> = [
  ...
  ['学习', /(学习|上课|课程|考试|作业|论文|阅读|培训|背单词|公开课|文献|答辩|实验报告|模拟考试|资格考试|读书会|听力|算法课|统计学)/],
  ['健康', /(健康|健身|运动|跑步|训练|复诊|体检|看病|服药|吃药|买药|牙医|眼科|康复|疫苗|心理咨询|睡眠|血压|游泳|瑜伽)/],
  ['出行', /(出行|旅行|旅游|交通|航班|飞机|高铁|火车|车票|机票|打车|坐车|出差|机场|车站|酒店|值机|行李|路线|租车|签证|景点|退票|换乘|行程|接机)/],
...
function parseDateRange(text: string, today: LocalDate): [LocalDate, LocalDate] | null {
  ...
  while ((match = pattern.exec(text))) {
    ...
    if (start && end && compareDate(end, start) >= 0) return [start, end];
  }
  return null;
}
...
function extractLocation(text: string): string | null {
  const match = text.match(/(?:在|地点(?:是|为)?|地址(?:是|为)?)([^，,。.!！?？]{2,40}?)(?:开会|会议|见面|上课|办事|$)/);
  if (!match) return null;
  const location = match[1].replace(/[，,。.!！?？的]+$/g, '');
...
function extractTitleAndDescription(text: string): [string, string | null] {
  ...
  let titleText = text;
  const descMatch = text.match(/(?:要)?(?:讨论|聊|沟通|确认|备注|内容是|主题是|关于)(.+)$/);
...
export function parseLocalScheduleText(text: string, referenceDate = new Date()): ParseResult | null {
  ...
  let parsedDate = parsedRange ? parsedRange[0] : parseExplicitOrRelativeDate(normalized, today);
  const endDate = parsedRange ? parsedRange[1] : null;

  let match = normalized.match(/每(?:周|星期|礼拜)([一二三四五六日天1-7])/);
  ...
  } else if ((match = normalized.match(/每月([0-9一二两三四五六七八九十]{1,3})(?:号|日)/))) {
    eventType = 'monthly';
    const day = Math.max(1, Math.min(31, cnToInt(match[1])));
    parsedDate = { ...today, day: Math.min(day, daysInMonth(today.year, today.month)) };
  } else if ((match = normalized.match(/每年([0-9一二两三四五六七八九十]{1,2})月([0-9一二两三四五六七八九十]{1,3})(?:号|日)/))) {
    eventType = 'yearly';
    const month = Math.max(1, Math.min(12, cnToInt(match[1])));
  ...
  const parsedTime = parsedTimeRange ? parsedTimeRange[0] : parseTime(normalized);
  ...
  return {
    ...
    parse_source: 'rules',
    confidence: 0,
    needs_clarification: !hasDate || (!hasTime && !effectiveEndDate),
    clarification_question: effectiveEndDate && !hasTime ? null : quickClarificationQuestion(hasDate, hasTime),
  };
}
