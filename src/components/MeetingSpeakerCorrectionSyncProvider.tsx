...
import { subscribeMeetingSpeakerCorrectionSync } from '../application/meeting/speakerCorrectionSyncTrigger';
import type { ScopeKey } from '../domain/meeting';
import { diagnosticWarn } from '../services/diagnostics';
import { drainMeetingSpeakerCorrectionSync } from '../services/meetingSpeakerCorrectionSync';
import { useAuth } from '../store/AuthStore';

export function MeetingSpeakerCorrectionSyncProvider({
  children,
  ...
  useEffect(() => {
      ...
      void (async () => {
        try {
          while (active && requested) {
            ...
            try {
              const result = await drainMeetingSpeakerCorrectionSync({
                scopeKey,
                accessToken,
          ...
          if (active) diagnosticWarn('[speaker-correction-sync] drain failed', error);
        } finally {
          running = false;
    ...
    const unsubscribe = subscribeMeetingSpeakerCorrectionSync(requestedScope => {
