...
状态：绑定 Marker/时间点的短文字与相册照片，以及默认关闭的分层分享，已形成游客/账号作用域隔离的本机纵向闭环；附件尚未接入整理输入或账号同步，因此 `ATT-01` 记为部分完成。本文件记录 migration v21、私有文件生命周期、会议详情/分享入口和保留数据模拟器实测，不把老记专属附件页描述成飞书已有功能。
...
- Summary 当前没有附件消费者。后续接入必须由用户独立显式勾选并写入 input fingerprint；不能复用一次分享授权，也不能以“当前有附件”为由自动读取。
...
2. 当前没有附件 outbox、服务端 schema、跨设备文件传输、冲突或配额合同，不能宣称账号同步。
3. 没有相机入口、视频/通用文件附件、批量选择、拖拽排序或附件编辑；这些都不是本基础纵切的隐含能力。
4. 没有 USB 真机、深色模式、字体缩放、25 MB 边界或存储不足设备验证；按轻量目标留到功能批次/候选版，而不反向抹掉已完成的本机纵切。
