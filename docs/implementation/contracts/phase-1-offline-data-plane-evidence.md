...
Phase 1 仍在进行中，不满足退出条件。当前批次完成了 SQLite v2 基础不变量、创建与独立阶段用例、旧 Store 双读/增量影子写，以及按原生 session 和作用域执行的录音 journal 对账。界面仍读取旧 Store，Transcript 和 Summary 也尚未切换为 canonical repository 写入。
...
- 大小：89,559,891 bytes
- SHA-256：`10be198aef0232bfcd4997647542090973183193ca4a344d33ab3104ad916332`
- 构建时间：`2026-07-22 18:04:58 +0800`
...
3. Transcript/Summary 仍由旧缓存写入，需先补 canonical repository 写接口和一致性对账，再考虑 SQLite read cutover。
4. UI 仍由旧 Store 提供事实；在 canonical 数据完整、真机升级计数和故障恢复通过前，不得把 SQLite 切为 UI 读源。
