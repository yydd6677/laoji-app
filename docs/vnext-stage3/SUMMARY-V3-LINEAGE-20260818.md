...
- 新增 `tools/vnext/evaluate_facts_v3_real_holdout.py`，对 8 个不同主题字幕窗口执行真实
  ...
  prompt 或规则。最新回放报告为 `facts-v3-real-holdout-20260818.json`，当前 `6/8` 通过。
- 失败样本暴露了两类真实问题：高覆盖长窗口可能只产出过少事实，议题覆盖不足；模型偶尔把
  战略方向输出成中适配行动。已在通用验证中将无明确有限交付边界的“加强/建立体系/持续推进”等
  宏观候选降为 `schedule_fit=low`，不能直接进入日程；这不是样本关键词门禁。
- 已补强 Facts prompt 的闭合检查和结构修复指引；单独重跑此前 JSON 截断样本已通过，但完整
  8 样本的事实支持率、长会议覆盖和人工行动有效性仍未满足 Stage 3 退出门，不能切 capability barrier。
