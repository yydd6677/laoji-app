...
- 继续审查设备模式的可达入口：会议详情的讲话人行不再被账号条件隐藏；讲话人修改面板改为无账号读取本机资料，并允许在明确同意后把 `future_profile` 修正保存为本机 `local_only`；设备模式同时隐藏只支持账号的“共享待办”入口。服务器兼容接口未删除，静态门禁和 TypeScript 已通过；本地已生成候选 release，但未安装到任何设备或做真机验收，详细记录见 [`docs/device-primary-visible-entry-audit-20260810.md`](device-primary-visible-entry-audit-20260810.md)。
