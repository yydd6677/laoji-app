...
- 真机安装烟测已完成：候选 release 已覆盖安装到 USB 设备 `825f509d`，`com.laoji.app` 显示 `versionName=1.0.6`、`versionCode=106`，更新时间为 `2026-08-10 10:28:57`；启动后进程保持运行并位于前台，最近日志未出现 `FATAL EXCEPTION`、`laoji_start_auth` 或未捕获 React Native 异常。该结果只证明安装/启动，不等于完整的真机日程、录音、转写、整理、问答和回收站验收。
- 继续审查设备模式的可达入口：会议详情的讲话人行不再被账号条件隐藏；讲话人修改面板改为无账号读取本机资料，并允许在明确同意后把 `future_profile` 修正保存为本机 `local_only`；设备模式同时隐藏只支持账号的“共享待办”入口。服务器兼容接口未删除，静态门禁和 TypeScript 已通过；本地候选 release 已完成真机安装/启动烟测，但未做完整真机功能验收，详细记录见 [`docs/device-primary-visible-entry-audit-20260810.md`](device-primary-visible-entry-audit-20260810.md)。
...
- 真实样本已在 `emulator-5562` 完成；候选 release 已在 USB 真机完成安装/启动烟测，完整真机功能验收仍未完成。
...
状态：部分完成；设备切片、日程 WebSocket、三服务重启健康恢复、地址设备鉴权、媒体矩阵、持久任务恢复、公网实时+后台并发 p95 以及真机安装/启动烟测已通过，问答/声纹质量和完整真机功能验收仍延期。

剩余工作（不重复已完成项）：

1. 实时+后台公网并发 p95 已达到计划门槛；后续性能工作可作为长时 soak 或真实多人样本优化，不再阻塞本计划的混合并发验收。
2. 按用户决定继续延期会议问答专题和真实声纹纵向质量验收；真机已完成安装/启动烟测，但完整功能验收仍待后续安排。
...
以下任一项未完成时，整体计划只能写“部分完成”：问答专题未验收、真实声纹纵向质量未验收、完整真机功能验收未完成、或摘要/问答历史断言尚未更新。实时+后台公网并发 p95、媒体矩阵、任务重启恢复、旧运行时审计、公网入口健康状态、日程解析合同和模板 revision 不再列为未完成项。
