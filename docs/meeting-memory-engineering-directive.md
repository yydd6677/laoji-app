...
| 结构化整理、版本、引用、行动项 | 本机/线上闭环；服务端独立 Summary task、四模板与 additive 同步框架运行 | `general@1` 完成非平凡 durable/引用/移动端跳转；四模板完成真实任务；历史授权完成利用、未选隔离、引用隔离、幂等与 durable 闭环 | 只续线上版本列表、更广真人样本和附件授权质量；ASR 的 GPU/NVML 问题另案处理 |
...
| SERIES-01 | 重复会议系列记忆 | 本机/线上闭环；真实账号决定/行动历史利用、未选隔离、引用隔离、幂等与 durable 已运行 | 跨移动设备 action 更新抽查 |
