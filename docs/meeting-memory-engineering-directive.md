...
> 规划估算：核心 P0/P1 框架约完成 100%，完整优化登记表（含线上闭环和 P2）约完成 98%。这是功能工作量估算，不是发布通过率；物理双机、真实模型质量与候选版集中验收仍单独计算。
...
| 结构化整理、版本、引用、行动项 | 本机闭环；服务端独立 Summary task 和 additive 同步框架运行 | `general@1` 非平凡真实任务约 92.3 秒成功，得到 3 条决定、7 条待办和 9 个有效引用；移动端完成 durable 恢复、引用跳转与重复响应幂等 | 只续另外三个模板、线上版本列表和更广真人样本；ASR 的 GPU/NVML 问题另案处理 |
...
| 通知、Widget、Tile、模板、系列记忆 | 本机纵切完成 | 模拟器、运行模板服务和 `general@1` 真实模型结果；另外三个模板、App Lock/真机待集中验收 | 不重做页面；只补剩余模板真实运行与发现的缺陷 |
...
> Phase 4 已形成连续纵向闭环：schema v2 sections/citations、immutable version、受保护版本不自动覆盖、本机候选/历史版本选择、snapshot、可定位引用、行动项创建/编辑/完成/忽略恢复/来源/提醒/后续日程，以及 action outbox/pull/冲突选择均已接通。SUM-02 的 segment-ID prompt、compact/Map-Reduce 来源保留、canonical 时间/quote 校验、唯一 quote 回填、漂移哈希与来源感知 action identity 已在目标 18020 运行。两个账号会话已覆盖 action ACK/pull、真实 409/412 与显式版本选择；`general@1` 又以导入的 10 段 final Transcript 取得 3 条决定、7 条待办、9 个有效引用及移动端精确跳转，冷启动重复响应保持幂等。该样本不是自动 ASR，也不替代物理双机；全账号 change feed、全局 `sync_cursor`、batch、action tombstone、线上版本列表和另外三个模板仍未完成。验证继续遵循“轻测试、轻校验”，严格样本与归档门禁后置。
...
| SUM-01 | 有序结构化整理结果 | 本机/线上纵切闭环；`general@1` 非平凡结构、durable 恢复和幂等当前版本已运行 | 另外三个模板、更多真人样本和线上版本列表 |
...
| TPL-01 | 四个内置模板 | 本机闭环；服务端运行，`general@1` 已有真实模型输出 | `one_on_one@1`、`project_sync@1`、`interview@1` 的真实模型输出 |
...
“带入我的笔记”只授权写入私人笔记，不自动授权把内容发送给模型。当前 Summary carry-forward 在模板选择后重新查询候选：默认零选择，取消终止生成，“不引用”显式继续；只有独立勾选的项目进入请求。授权 request ID、项目快照、模板和 Transcript 固化进 input fingerprint 与 pending task；恢复、任务丢失重提及结果校验保持同一 identity。账号服务校验来源会议归当前用户；历史内容只作为背景，不得冒充本场 Transcript 或生成本场引用。目标服务现已运行，`general@1` 无历史授权的非平凡生成已通过；历史内容的真实利用质量和未选内容隔离仍需带候选资料的独立样本，不能由本次空授权结果代替。
...
5. 普通包账号根写与独立账号上传写均已开启；上传和逐资产转写每个认证会话仍要求 fresh `recording_assets_v2=true`，保留 capability fail-closed 和旧版本回滚窗口。per-asset provenance、v26 transcription job、独立 Summary 恢复/成功引用及 speaker correction/profile 已完成；批次 B 只剩 GPU 自动 ASR、另外三个模板与第二移动设备抽查。
...
Marker 账号同步完成 + general Summary 真实模型完成 ── 其余模板/自动 ASR ── 单次候选版物理设备收口
```
