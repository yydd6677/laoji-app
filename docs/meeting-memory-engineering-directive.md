...
> 规划估算：核心 P0/P1 框架约完成 100%，完整优化登记表（含线上闭环和 P2）约完成 99%。这是功能工作量估算，不是发布通过率；自动 ASR、物理双机与候选版集中验收仍单独计算。
...
| 结构化整理、版本、引用、行动项 | 本机/线上闭环；服务端独立 Summary task、四模板与 additive 同步框架运行 | `general@1` 完成非平凡 durable/引用/移动端跳转；三个非默认模板完成真实任务，定向访谈生成四类 section 与 5 个 canonical 引用 | 只续线上版本列表、更广真人样本和历史/附件授权质量；ASR 的 GPU/NVML 问题另案处理 |
...
| 通知、Widget、Tile、模板、系列记忆 | 本机纵切完成；四模板运行闭环 | 模拟器、运行模板服务、四模板真实模型结果和定向访谈来源；App Lock/真机待集中验收 | 不重做页面；只补历史授权质量、真机与发现的缺陷 |
...
> Phase 4 已形成连续纵向闭环：schema v2 sections/citations、immutable version、受保护版本不自动覆盖、本机候选/历史版本选择、snapshot、可定位引用、行动项创建/编辑/完成/忽略恢复/来源/提醒/后续日程，以及 action outbox/pull/冲突选择均已接通。SUM-02 的 segment-ID prompt、compact/Map-Reduce 来源保留、canonical 时间/quote 校验、唯一 quote 回填、漂移哈希与来源感知 action identity 已在目标 18020 运行。两个账号会话已覆盖 action ACK/pull、真实 409/412 与显式版本选择；四模板均已有真实模型成功结果，定向访谈又生成全部四类 section 与 5 个 canonical 引用。该样本不是自动 ASR，也不替代物理双机；全账号 change feed、全局 `sync_cursor`、batch、action tombstone 和线上版本列表仍未完成。验证继续遵循“轻测试、轻校验”，严格样本与归档门禁后置。
...
| SUM-01 | 有序结构化整理结果 | 本机/线上纵切闭环；四模板结构、durable 恢复和幂等当前版本已运行 | 更多真人样本、历史/附件授权质量和线上版本列表 |
...
| TPL-01 | 四个内置模板 | 本机/线上纵切闭环；四模板真实模型输出，定向访谈 section/引用已运行 | 快速连续切换、历史授权质量、跨设备与 USB 抽查 |
...
服务端采用严格 `id@revision` 白名单，把模板合同归一化为 schema v2。真实 `FinalSummary` 表尚无模板列，因此兼容阶段将 v2 envelope 存入输出 JSON 的 `_laoji_structured_summary`，不伪造已完成的数据库迁移。无历史/附件的四模板现使用 4B 动态 JSON schema 精简路径，模板专属 section 携带同段 ID/逐字 quote 并经过 canonical 校验；带授权任务仍走完整上下文管线。目标 18020 已完成四模板真实模型任务，定向访谈覆盖四 section 与来源；剩余是历史授权质量、跨设备和真机，不再是模板运行缺口。合同和设备边界见 [`implementation/contracts/phase-6-template-evidence.md`](implementation/contracts/phase-6-template-evidence.md)。
