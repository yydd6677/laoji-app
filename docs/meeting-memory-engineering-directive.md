...
> 规划估算：核心 P0/P1 框架约完成 98%–99%，完整优化登记表（含线上闭环和 P2）约完成 95%–96%。这是功能工作量估算，不是发布通过率。
...
| 录音结束、Transcript 搜索回听、后台播放 | 本机主链、RecordingAsset v2 上传/下载、逐录音 provenance/回听、每资产 job 和独立 Summary 恢复闭环 | 编译、目标服务双资产 GPU 转写与 14 项真实 HTTP 断言、v25/v26 升级、真实 transcript retry、Summary success/重启恢复；当前共享 GPU 驱动失配 | 接 completeness/reprocessed；GPU 恢复后补自动转写成功，候选版一次验收 |
...
| 文件导入、Marker、片段、分层分享、删除/回收站 | 音视频导入、已有会议多资产加入、附件分享、本机 WAV 片段、软删除/恢复及双端到期物理清理纵切完成 | 视频/双录音模拟器、真实 MP4 上传/抽取/逐资产转写、v27 本机窄合同、18020 后台任务；片段 v23/真实 WAV | 导入只续格式矩阵/第二设备/USB；功能量续 Marker 链接和非 WAV 异步片段；删除只续组合/真机抽查 |
...
| QA、跨会议组织、轻协作、媒体片段、时间点附件 | QA-01 与 COLLAB-01 已形成线上纵切；ORG-01 已聚合人物、用户标签和当前整理主题；ATT-01 本机附件；CLIP-01 本机 WAV 纵切 | QA 真实模型/账号持久化；COLLAB 模拟器所有者 + 匿名第二客户端；ORG v20 + current Summary 只读派生；ATT v21；CLIP v23 | QA 只续完整移动端 Transcript 路径；COLLAB 只续第二台移动设备/USB；ORG 只续账号标签同步；ATT/CLIP 续线上或非 WAV 边界 |
...
7. 音频文件/系统分享导入、Marker、内容级分享、安全默认、私人笔记二次确认和本机回收站界面。
...
| C：补齐尚缺功能量 | P0/P1 尾项和五个 P2 纵切均有真实入口、持久化与失败状态 | 视频导入/已有会议加入、QA/轻协作线上纵切和整理主题聚合已完成；继续账号标签同步、Marker 链接、非 WAV 片段和照片多模态/账号附件 | 不先建设复杂 Agent、团队权限树或完整编辑器 |
...
> Phase 8 已完成 ORG-01 的三组本机纵向切片。第一组以 migration v20 接通作用域隔离的用户标签、创建/分配/改名/同名合并/删除，以及标题、标签、我的笔记、active Transcript、current Summary 和 Action 的本机分源索引；Android 以 FTS5 trigram/短词 LIKE 保留命中来源、source ID 和 Transcript 时间。第二组直接从 active Transcript 派生人物、从用户标签派生主题：稳定 `speaker_profile_id` 优先；无 profile 只按 scope 内精确姓名临时聚合并标“未确认”；匿名编号和泛化标签不会成为人物。第三组由独立 `meetingAutomaticTopicsV1` flag 只读提取 current ready/stale Summary 的 `topics` section，用户编辑文本优先；按 NFKC/折叠空白/小写稳定聚合，每场最多 12 个，拒绝泛化标题、控制字符、结构外壳、超长行和重复项，且从不写入 `meeting_tags`。分类页用“用户标签 / 整理主题”安静徽标保持来源可见。模拟器夹具验证 1 个用户标签、2 个整理主题、重复收敛、损坏格式拒绝和详情跳转；原数据库三文件随后按 SHA-256 精确恢复。Folder 仍保留需求门槛，账号标签同步和 USB 真机尚未完成，因此 ORG-01 仍记为部分完成。证据见 [`implementation/contracts/phase-8-organization-search-evidence.md`](implementation/contracts/phase-8-organization-search-evidence.md)。
> Phase 8 也完成 ATT-01 的三组本机纵向切片：migration v21、绑定 Marker 与时间点的短文字/相册照片、应用私有文件复制、附件总览、单项删除、默认关闭的分层分享，以及文字附件逐次授权进入整理结果。Marker 删除只把附件来源置空并保留时间点；可恢复删除保留附件，永久删除清理记录和私有文件。整理授权与分享授权分离且默认关闭，保存 request ID、附件 ID/时间点/内容/hash/更新时间并进入 v4 task fingerprint；恢复前重新核对当前附件，变化后不静默重提。服务端 overlay 把文字附件纳入 dedupe/prompt/结果 identity，并禁用不接收附件上下文的短问候与 compact 快路径；照片能力明确为 false，不以文件名冒充视觉输入。模拟器已验证零选择、照片禁用、取消后复位、当前服务未声明能力时中文 fail closed、“不使用”继续和原数据精确恢复。目标服务源码未同步或运行，真实模型、账号附件同步、照片多模态和 USB 真机仍未完成，因此 ATT-01 记为部分完成。证据见 [`implementation/contracts/phase-8-timepoint-attachments-evidence.md`](implementation/contracts/phase-8-timepoint-attachments-evidence.md) 与 [`implementation/contracts/phase-8-summary-attachments-evidence.md`](implementation/contracts/phase-8-summary-attachments-evidence.md)。
...
5. video ingest、远端视频音轨处理和已有会议显式加入已经完成移动端/18020 纵切；reprocessed Transcript 生产入口、ORG-01 的账号同步/实验 topic、Marker 链接、非 WAV 片段和照片多模态/账号附件仍有真实功能量未完成。COLLAB-01 只剩第二台移动设备/USB 证据；删除到期物理清理只剩真实组合与真机抽查。
...
| SRC-01 | 日程、临时录音、文件导入统一为 `MeetingNote` | 本机与账号音视频、多录音、per-asset Transcript 纵切闭环 | 非 WAV 片段/账号附件继续复用同一模型 |
...
| TRN-01 | 搜索、跳转、按录音来源回听、高亮、复制/分享 | 本机闭环；per-asset segment provenance 已运行/模拟器闭环 | 服务端 completeness/reprocessed 入口；候选版长录音抽查 |
| ACT-01 | 行动项编辑、完成、提醒/日程、来源 | 本机闭环；运行 action pull 与协作者 revision 收敛完成 | 非协作 action 的真实冲突选择、提醒跨设备收敛 |
| SUM-01 | 有序结构化整理结果 | 本机闭环；独立服务任务、schema v2 与重启恢复已运行 | 非平凡真实会议的 section/引用质量和线上版本列表 |
...
| ORG-01 | 标签/Folder、多场检索与聚合 | 部分完成；标签、分源搜索、人物、用户标签主题与当前整理主题本机闭环 | 账号标签同步；Folder 仅在需求成立时添加 |
...
当前 v25 provenance、v26 每资产任务账本、双录音模拟器定位和目标 18020 的 14 项真实 HTTP provenance 断言已完成；保留数据模拟器又实际创建并恢复了账号 transcription job。共享 GPU 驱动/NVML 失配使本轮 job 最终保持 `failed_retryable`，成功 combined Transcript、第二移动设备和 USB 真机仍后置。证据见 [`implementation/contracts/batch-b-recording-assets-v2-evidence.md`](implementation/contracts/batch-b-recording-assets-v2-evidence.md)。
...
当前落地：第 1、3、4、5 项本机纵切已完成。整理主题由独立 flag 从当前可读 Summary 版本派生，复用既有模型产物和用户编辑保护，不新建并行模型任务、表或标签写入；页面明确区分“用户标签/整理主题”。第 2 项仍等待大量会议的真实需求证据，账号标签同步是 ORG-01 剩余功能量。实现与模拟器边界见 `implementation/contracts/phase-8-organization-search-evidence.md`。
...
文字附件现以独立“选择附件”sheet 接入 Summary：每次生成默认零选择，“取消”终止，“不使用”以空附件授权继续；授权不复用分享选择。pending task 保存 request ID、附件 ID、kind、时间点、正文、SHA-256 与更新时间，全部进入 v4 fingerprint；任务恢复和提交前重新读取 SQLite，附件删除或变化后丢弃旧任务。服务端只有在 fresh capability 明确 `summary_attachments_text=true` 时才接收内容，并回传相同 `attachment_request_id`；授权请求禁用不接收额外上下文的 compact/短问候路径。当前模型链没有可靠视觉输入，因此 `summary_attachments_image=false`，照片行可见但不可选，不得以文件名或时间点冒充已读取图片。账号附件同步、真实模型文字附件质量、照片多模态和真机仍待后续。
...
当前目标 18020 的运行响应已明确：`meeting_notes_v2`、结构化整理/引用、文字附件整理、单场问答、action/pull/collaboration、`manual_notes_v2`、`occurrence_links_v2`、`recording_assets_v2`、`speaker_corrections`、`speaker_profiles_v2` 与 `speaker_reprocess_v1` 为 true，`soft_delete_days=30`；`summary_attachments_image` 与全局 `sync_cursor` 为 false，`media_import=null`。客户端仍逐项按本次实时响应控制会改变数据的操作，不能由某个 broad flag 推导 upload、speaker 或全局同步能力。
...
5. 普通包账号根写与独立账号上传写均已开启；上传和逐资产转写每个认证会话仍要求 fresh `recording_assets_v2=true`，保留 capability fail-closed 和旧版本回滚窗口。per-asset provenance、v26 transcription job、独立 Summary 恢复及 speaker correction/profile 已完成；批次 B 只剩 GPU 恢复与第二移动设备抽查。
...
- `ORG-01`：标签、跨会议本机分源检索、人物、用户标签主题和 current Summary 整理主题聚合已完成；整理主题不写回用户标签。继续账号标签同步；Folder 只在大量会议需求成立时添加。
