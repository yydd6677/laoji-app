...
| 文件导入、Marker、片段、分层分享、删除/回收站 | 音视频导入、已有会议多资产加入、Marker 本机事务/附件分享、可撤销文字链接、本机 WAV 与非 WAV 异步片段、软删除/恢复及双端到期物理清理纵切完成；Marker 账号同步尚未实现 | 视频/双录音模拟器、真实 MP4 上传/抽取/逐资产转写、v29/v30 保留数据模拟器、18020 共享/清理/片段任务；M4A/MP4 异步抽取及原生 WAV 校验；双模拟器实测 Marker 未随会议恢复 | 先补 Marker schema/API/outbox/pull；导入只续格式矩阵/第二设备/USB；片段、分享、删除只续跨设备与真机抽查 |
...
| C：补齐尚缺功能量（主体收口，剩 Marker 账号同步） | P0/P1 尾项和五个 P2 纵切均有真实入口、持久化与失败状态 | 视频导入/已有会议加入、附件分层分享、可撤销分享链接、QA/轻协作/账号标签目录、账号附件对象同步、非 WAV 异步片段、整理主题聚合与 reprocessed Transcript 移动端/持久 overlay 纵切均已完成；Marker 本机功能完成但服务端同步仍缺 | 不先建设复杂 Agent、团队权限树或完整编辑器；只复用现有 outbox/pull 补 Marker，不新造同步系统 |
...
| 完成条件 | 当前证据 | 判断 |
|---|---|---|
| 第 4 节无 `进行中/部分完成/未开始` | ATT-01 已补齐 18020 与双模拟器附件对象闭环；但 MRK-01 的服务端 schema/API/outbox/pull 尚不存在 | **未满足完整功能量口径**；Marker 账号同步是确认后的真实缺口，图片模型只是外部证据边界 |
...
| 默认候选 APK、回溯提交与稳定标签 | Preview `ee24ffc2…cc97e4`（91,009,536 bytes，v104）已在两个独立模拟器逐字节安装，对应移动端提交 `b3ea399`；`stable-before-meeting-memory-roadmap` 仍为 `cde96f9d5266961e380957893ecba39855aea39b` | **满足可回溯候选包口径**；当前 hash 的 USB 安装仍待 |
...
5. video ingest、远端视频音轨处理、已有会议显式加入、附件分层分享、可撤销文字链接、CLIP-01 非 WAV 异步片段和 ORG-01 账号标签目录/整理主题已经完成移动端/18020 纵切。reprocessed Transcript 已完成移动端生产入口、v31 恢复任务、不可变版本与持久服务端 overlay，只剩线上真实 job 证据。账号附件已完成移动端、18020、真实测试账号与双模拟器文字附件收敛；照片多模态仍缺真实视觉模型。Marker 仅完成本机能力，账号 schema/API/outbox/pull 尚未实现，是本路线仍存在的功能缺口。
...
| MRK-01 | Marker、会后跳转、转行动项/分享 | **部分完成**；本机闭环，已与附件、WAV/非 WAV 片段和具体 RecordingAsset 来源打通；双模拟器确认另一实例不会恢复 Marker | 复用既有同步基础补服务端 schema/capability、幂等 upsert/tombstone、移动端 outbox/pull 与冲突边界；再做第二实例抽查 |
...
当前只有 Action 的单项 upsert/list/conflict 纵切和本机 Marker 事务已经实现。`POST/PATCH /v2/meeting-notes/{id}/markers`、Marker 服务端表/capability、移动端 outbox/pull/tombstone 仍未部署；双模拟器已实际观察到来源设备的 Marker 不会在另一实例恢复。上述 Marker endpoint 不能继续被当作“已实现接口清单”，MRK-01 必须先复用现有 root/action/attachment 的账号隔离与幂等模式补齐，再进入候选抽查。附件可凭自身 `meeting + position_ms` 跨设备出现，不构成 Marker 同步替代方案。
...
- `MRK-01`：本机创建/定位/删除/转待办/分享及与附件、片段的关联已完成，但账号 Marker 同步未实现；必须复用既有 canonical/outbox/pull 基础补 schema、capability、幂等 upsert/tombstone 和第二实例恢复，不得由 ATT 的独立 `position_ms` 同步冒充完成。
