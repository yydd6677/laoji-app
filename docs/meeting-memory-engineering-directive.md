...
| 默认候选 APK、回溯提交与稳定标签 | Preview `634999b9…21b716c`（91,091,308 bytes，v104，构建于 `2026-07-29 02:26:41 +0800`）内容对应 `81fe86d`，已保留数据覆盖安装到仅供老记候选验证的 `emulator-5556`，设备内 `base.apk` 哈希完全一致；v36、section 编辑/引用覆盖、双资产重转写、Summary lineage、正常软键盘、冷启动恢复及隐私只读检查无 SQLite/React Native/native fatal。`stable-before-meeting-memory-roadmap` 仍为 `cde96f9d5266961e380957893ecba39855aea39b` | **满足当前可回溯候选包与保留数据模拟器口径**；USB 已断开，当前 hash 的真机安装、物理双机与稳定包覆盖升级重跑仍待 |
