...
describe('API config', () => {
  it('uses the internal test server by default', () => {
    ...
    expect(getApiConfig().laojiApiBase).toBe('http://183.36.243.124:18035');
    expect(getApiConfig().meetingApiBase).toBe('http://183.36.243.124:18020');
  });

  it('rejects plain HTTP production endpoints', () => {
    process.env.APP_ENV = 'production';
    process.env.EXPO_PUBLIC_LAOJI_API_BASE = 'http://183.36.243.124:18035';
