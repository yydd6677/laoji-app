...
// UI-SHELL-RESELECT-001 / CAL-DAY-PAGER-001 / CAL-ALLDAY-001 / CAL-ALLDAY-EXPAND-001 / CAL-TIME-PRECISION-001 /
// CAL-MONTH-EXPAND-001 / CAL-MONTH-SPAN-001 / CAL-DAY-COMPOSE-001: API 35 runtime coverage for the source-derived native behavior.
...
class CalendarSurfaceInstrumentedTest {
    ...
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      ...
      scenario.onActivity { activity ->
        ...
        assertEquals(
          ...
          allDayPage.visibleLabelsForTest(),
        )
        val instanceLayerBeforeExpand = requireNotNull(allDayPage.instanceLayerForTest())
        assertTrue(allDayPage.performOverflowClickForTest())
        assertTrue(surface.dayAllDaySectionView.isExpanded())
        assertTrue(
          "all-day instance layer must survive the source-shaped height transition",
          instanceLayerBeforeExpand === allDayPage.instanceLayerForTest(),
        )
  ...
  @Test
  fun CAL_DAY_COMPOSE_001_usesSourceRoundedInstanceLayoutInProductionView() {
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      val day = selectedDay()
      val sourceLayoutEvent = timedEvent(day).copy(
        instanceLayout = CalendarInstanceLayout(
          xOffsetPercent = 25f,
          yOffsetPercent = 42f,
          widthPercent = 50f,
          heightPercent = 5f,
      ...
      val surfaceRef = addDaySurface(
        scenario,
        calendarSnapshot(day).copy(events = listOf(sourceLayoutEvent)),
        onOpened = opened::set,
      )

      waitForDaySurface(scenario, surfaceRef)
      scenario.onActivity { activity ->
        val timeline = centerTimelinePage(surfaceRef.get(), day).timelineCanvas
        val evidence = requireNotNull(FeishuEvidenceRuntime.ref(timeline))
        assertEquals("CAL-DAY-COMPOSE-001", evidence.id)
        assertEquals("day-timed-event-layer", evidence.role)

        val node = requireNotNull(timeline.accessibilityNodeProvider.createAccessibilityNodeInfo(1))
        val actual = Rect().also(node::getBoundsInParent)
        val eventAreaWidth = timeline.width - timeline.rulerWidth() - timeline.rightPadding()
        val content = DayEventVisualContract.instanceLayoutRect(
          layout = requireNotNull(sourceLayoutEvent.instanceLayout),
          eventAreaLeft = timeline.rulerWidth(),
          eventAreaWidth = eventAreaWidth,
          timelineTop = timeline.topPadding(),
          timelineHeight = timeline.contentHeight(),
        )
        val expected = Rect(
          floor(content.left).toInt().coerceAtLeast(0),
          floor(content.top - timeline.scrollOffset()).toInt().coerceAtLeast(0),
          ceil(content.right).toInt().coerceAtMost(timeline.width),
          ceil(content.bottom - timeline.scrollOffset()).toInt().coerceAtMost(timeline.height),
        )
        assertEquals(expected, actual)

        val bitmap = Bitmap.createBitmap(timeline.width, timeline.height, Bitmap.Config.ARGB_8888)
        timeline.draw(Canvas(bitmap))
        val sampleX = (actual.right - activity.dp(2f)).coerceAtLeast(actual.left + 1)
        assertEquals(
          CalendarUi.palette(activity).eventFill,
          bitmap.getPixel(sampleX, actual.centerY()),
        )
        bitmap.recycle()

        assertTrue(
          timeline.accessibilityNodeProvider.performAction(
            1,
            AccessibilityNodeInfo.ACTION_CLICK,
            null,
          ),
      ...
      assertEquals(sourceLayoutEvent.identity, opened.get()?.identity)
    }
  }
  ...
  fun headerAndAllDayGesturesDriveContinuousPagerAndKeepAdjacentHeightStable() {
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      ...
      scenario.onActivity {
        val surface = surfaceRef.get()
        assertTrue(centerAllDayPage(surface, day).performOverflowClickForTest())
      }
      waitUntil(scenario) {
  ...
  @Test
  fun CAL_MONTH_SPAN_001_rendersOneWeekLocalAccessibilityRectAcrossOccupiedColumns() {
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      val day = selectedDay()
      val monthStart = CalendarDateMath.monthStart(day)
      val gridStart = CalendarDateMath.monthGridStart(monthStart)
      val weekStart = gridStart + ((day - gridStart) / 7) * 7
      val span = CalendarEvent(
        sourceEventId = "span",
        occurrenceDate = "2026-07-13",
        title = "跨日安排",
        startEpochDay = weekStart + 1,
        endEpochDay = weekStart + 3,
        endEpochDayExclusive = weekStart + 4,
        startMinutes = null,
        endMinutes = null,
      ...
      val snapshot = calendarSnapshot(day).copy(events = listOf(span))
      val opened = AtomicReference<CalendarEvent?>()
      val pagerRef = addMonthSurface(
        scenario = scenario,
        day = day,
        onSelected = {},
        onOpened = opened::set,
      ...
      waitForMonthSurface(scenario, pagerRef, span.startEpochDay)
      waitUntil(scenario) {
        visibleWeekRow(pagerRef.get(), span.startEpochDay)
          ?.accessibilityNodeProvider
          ?.createAccessibilityNodeInfo(100) != null
      }

      val eventPoint = AtomicReference<ScreenPoint>()
      scenario.onActivity { activity ->
        val row = requireNotNull(visibleWeekRow(pagerRef.get(), span.startEpochDay))
        val evidence = requireNotNull(FeishuEvidenceRuntime.ref(row))
        assertEquals("CAL-MONTH-SPAN-001", evidence.id)
        assertEquals("month-week-row", evidence.role)

        val eventNode = requireNotNull(
          row.accessibilityNodeProvider.createAccessibilityNodeInfo(100),
        )
        assertNull(row.accessibilityNodeProvider.createAccessibilityNodeInfo(101))
        val actual = Rect().also(eventNode::getBoundsInParent)
        val gridLeft = activity.dp(MonthExpandedLayoutContract.GRID_START_MARGIN_DP).toFloat()
        val gridRight = activity.dp(MonthExpandedLayoutContract.GRID_END_MARGIN_DP).toFloat()
        val cellWidth = (row.width - gridLeft - gridRight) / MonthExpandedLayoutContract.DAY_PAGE_COUNT
        val expected = MonthExpandedLayoutContract.eventSpanBounds(
          gridStart = gridLeft,
          cellWidth = cellWidth,
          startColumn = 1,
          endColumn = 3,
          rightGap = activity.dp(MonthExpandedLayoutContract.EVENT_RIGHT_GAP_DP).toFloat(),
        )
        assertEquals(expected.left, actual.left.toFloat(), 1f)
        assertEquals(expected.right, actual.right.toFloat(), 1f)
        assertTrue(actual.width() > cellWidth * 2.5f)
        assertTrue(actual.left < gridLeft + 2f * cellWidth)
        assertTrue(actual.right > gridLeft + 3f * cellWidth)
        eventPoint.set(row.screenPoint(actual.exactCenterX(), actual.exactCenterY()))
      }

      injectTap(eventPoint.get())
      waitUntil(scenario) { opened.get()?.sourceEventId == "span" }
    }
  }
  ...
  fun monthExpansionSurvivesImmediateSelectedDateSnapshotRebind() {
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      ...
      scenario.onActivity {
        ...
        assertEquals(0f, selectedRow.translationY, 0f)
        assertEquals(selectedRow.top.toFloat(), selectedRow.y, 0.5f)
      }

      injectTap(monthDatePoint(scenario, pagerRef::get, targetDay))
      waitUntil(scenario, timeoutMs = 2_000L) {
        selected.get() == day - 1 && visibleMonthEventOwner(pagerRef.get()) == null &&
          monthTransitionSettled(pagerRef.get())
      }
    }
  }
  ...
  fun monthCrossRowRunsTwoLegsAndCrossMonthOrInterruptionCannotRestoreStaleSelection() {
    ActivityScenario.launch(CalendarSurfaceTestActivity::class.java).use { scenario ->
      ...
      scenario.onActivity {
        val owner = requireNotNull(visibleMonthEventOwner(pagerRef.get()))
        assertEquals(1f, owner.alpha, 0f)
        assertEquals(
          (visibleMonthPage(pagerRef.get()).readPrivateField("pendingSelection") as MonthExpandedSelection).row,
          owner.readPrivateField("boundRow"),
        )
      }
      ...
      assertEquals(1f, opening.eventOwnerAlpha, 0f)
  ...
  private fun addMonthSurface(
    ...
    onSelected: (Int) -> Unit,
    onOpened: (CalendarEvent) -> Unit = {},
    snapshot: CalendarSnapshot = calendarSnapshot(day),
