...
- 单日视图由日期头、全天区、三页日期 Pager、时间轴 Canvas、事件层和独立拖动层组合。全天区按飞书 `AllDayInstanceLayout -> PositionedViewLayout` 的所有权复用同一个实例绘制层，只更新数据、可见行和高度，不在展开动画中销毁/重建子 View。事件层的源码闭包为 `EventInstanceLayer -> EventInstanceDataMapper -> DayEventInstanceDrawableData -> EventInstanceDrawableData -> C153808d/C153806b/C153805a/C153809e`，布局输入来自 `InstanceLayout` 百分比矩形，并按源码逐边半向上取整到像素；source-shaped bridge 与生产 View 几何/像素/命中已测试，无矩形时才使用保留 2dp 间距的本地 lane fallback。真实服务矩形、`ColorIndex` adapter 和飞书运行 golden 未完成前不得宣称等价。可见标尺为 25 条整点线；默认时长与 15/30 分钟吸附属于不同合同。
- 年月 QuickChoose 位于日历页面树内，具有日期/年月双态、可点击 wheel、拖动和中断可逆动画。关闭态不得用 `GONE` 销毁测量结果；老记使用 `INVISIBLE` 保留面板尺寸并禁止触摸，内容仍按飞书合同平移到屏幕外。宿主在首次布局前预绑定当前日期与模式，首次标题点击不能依赖打开后的临时测量。
