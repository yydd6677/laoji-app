...
- 当前源码锁：386 个文件，其中通用壳层 38、日历 193、妙记 75、账号静态容器 8、资源 72。日历事件层本轮新增锁定 `rd3/C153805a..C153817m`、`yc3/C162788d`、`InstanceLayout` DTO 和 `EventInstance*` 数据映射闭包；月跨日条补锁 `InterfaceC152324b` 的 row/column/width 合同与 `C152323a` 的 Canvas 矩形公式；全天区补锁 `AllDayInstanceView` 与对应绘制 owner `C155129a`。
...
| 跨日条 | `C155132d -> InterfaceC152324b -> C152323a -> C153854f` | 先裁剪到当前七日矩阵，在起始列取首个空闲层，连续占用裁剪后的跨度，再由同一公式映射为周内单个矩形 | 保留 |
| 单日组合 | `SingeDayView -> SingleDayHeaderView + AllDayInstanceLayout + DayInstanceLayout` | 日期头、全天区、时间轴和事件层有独立所有权 | 保留 |
| 定时事件层 | `EventInstanceLayer -> EventInstanceDataMapper -> DayEventInstanceDrawableData -> EventInstanceDrawableData -> C153808d -> C153806b/C153805a/C153807c` | `CalendarViewService` 返回的 `InstanceLayout` 用百分比矩形映射到事件层，并按每条边半向上取整到像素；`C153808d.m522729h` 负责定时事件的 9/3dp 文本内边距与圆角 drawable 组合，`C153809e.m522734e` 负责 `ud_N500` 10% 按压遮罩；颜色由 `C162788d` 按事件色索引解析 | 源码闭包已锁；老记已有 source-shaped `InstanceLayout` bridge、fallback 间距和生产 View 几何/像素/命中测试，真实 `InstanceLayout`/ColorIndex adapter 与飞书运行 golden 尚未关闭 |
...
| 全天区 | `AllDayInstanceLayout` | 折叠最多三行，可展开；内部滚动与横向翻日分离；`PositionedViewLayout` 复用实例 owner，只更新 drawable 数据和高度 | 保留 |
| QuickChoose | `QuickChooseDatePanel -> QuickChooseDateComponent -> CalendarYearMonthPicker -> WheelView` | 外层四态；根面板始终保留在布局树，关闭时内容以 `translationY` 移出；日期/年月双态；可见行点击和滚动都 settle 并提交 | 保留 |
