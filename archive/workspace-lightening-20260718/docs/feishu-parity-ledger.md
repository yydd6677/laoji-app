...
| --- | --- | --- | --- | --- |
| `CAL-ALLDAY-001` | calendar | 已实现 | `keep` | 2 |
| `CAL-DAY-COMPOSE-001` | calendar | 已实现 | `keep` | 3 |
| `CAL-MONTH-EXPAND-HOST-001` | calendar | 已验证 | `keep` | 0 |
| `CAL-MONTH-SPAN-001` | calendar | 已实现 | `keep` | 1 |
...
- 源码锁：386 个文件，包含 common-shell 38、calendar 193、minutes 75、account-static 8、resources 72；本轮补齐日历定时事件 drawable/layout 闭包、月跨日条的 row/column/width 接口与 Canvas 映射公式，以及全天区 `AllDayInstanceView`/绘制 owner 闭包。
...
2. `UI-LEGACY-001`：当前 TS AST 报告仍发现 205 个未映射错误，Android UAST/Lint 报告包含 560 个问题，其中 464 个错误；本批次改动的全天区、单日时间轴与月视图 owner 文件均为 0 个 UAST 错误，旧 Android 表现层与孤立 TSX 尚未清零，Release 门禁继续阻断。
3. `UI-ROUTES-001`：16 个 Root 路由和 2 个 MainTabs 目的地尚未逐项绑定源码容器、转场、恢复和 View 树合同。
4. 三个 pilot proof 使用当前源码锁、manifest、AST/UAST/workspace 报告和 API 35 模拟器 54 项合并 instrumentation 签发，其中包含 QuickChoose 关闭态仍有有效面板/拖动尺寸、首次标题点击打开、隐藏日/月 owner 不受重选驱动、模拟 React 即时/运行中快照反馈不截断 motion、重复截止开关与循环滚轮，以及生产 Calendar/Minutes Android 根双向替换的回归用例；后续任一输入变化仍会立即使其过期。
...
- 月展开的跨行、跨月和动画关键帧尚未闭环；跨日连续条已补生产 View 树几何与命中测试，仍缺独立签名 proof 和飞书运行对照，暂不能关闭。
- 日期头、全天区、时间轴和事件层的同步 Pager motion 尚未闭环；全天区已按源码复用实例绘制 owner，展开高度变化不再销毁/重建 Canvas；单日定时事件已补 source-shaped `InstanceLayout` 的生产 View 几何、像素与命中测试，真实服务矩形和 `ColorIndex` 仍未接入。
- 标尺、默认时长、创建吸附和 resize 吸附已拆成独立源码合同；原先无效的 `dragPrecisionMinutes` 外部通道已删除。独立 View 树几何与真机手势闭证仍未关闭。
- 标尺与月展开已统一系统 12/24 小时格式；搜索、详情和编辑页面仍有固定 24 小时显示路径，尚未关闭完整跨页面合同。
- QuickChoose 根面板已从关闭态 `GONE` 改为持续测量的 `INVISIBLE`，宿主在首次布局前预绑定当前日期与模式；API 35 完整 54 项 instrumentation 已验证关闭态尺寸非零、首次打开和后续日期/年月交互。该适配等价于飞书“根留在树中、内容平移隐藏”，同时避免透明层拦截老记页面触摸。
